...
PATTERNS: list[Pattern] = [
    ...
    Pattern("base64_decode_exec", "obfuscation",
            ...
            [r"eval\s*\(\s*base64", r"exec\s*\(\s*base64",
             r"base64\.b64decode.*eval", r"base64\.b64decode.*exec"], [".py"]),
    ...
    Pattern("privilege_escalation", "system_exec",
            "Silently gaining admin/root — no legitimate model needs this", 97,
            [r"runas\s*/user:Administrator", r"Start-Process.*-Verb\s+RunAs",
             r"sudo\s+-S"], [".bat", ".ps1", ".py", ".sh"]),
    ...
    Pattern("unsafe_pickle_load", "model_exploit",
            ...
            [r"pickle\.loads?\s*\(", r"__reduce__", r"__reduce_ex__"], [".py"]),
    ...
    # Fix 1: torch.load regex REMOVED — handled by AST only to prevent dedup
    Pattern("config_auto_execute", "hf_exploit",
            "from_pretrained with trust_remote_code — remote execution risk", 75,
            [r"from_pretrained\s*\([^)]*trust_remote_code"], [".py", ".md"]),
    ...
    def _scan_file(self, path: str, content: str, ext: str) -> list[PatternMatch]:
        ...
        for pattern in self.patterns:
            ...
            for regex in pattern.regexes:
                found = self._regex_scan(content, regex, path, pattern, in_test)
                ...
                if "weights_only" not in {kw.arg for kw in node.keywords}:
                    ...
                    if key not in reported:
                        matches.append(PatternMatch(
                            ...
                            file_path=path, line_number=node.lineno, severity=88,
...
def calculate_trust_score(matches: list[PatternMatch]) -> int:
    if not matches:
        return 100
    for m in matches:
        if m.category in _INSTANT_DANGEROUS and m.severity >= _INSTANT_THRESHOLD:
            return 15
    score = 100.0
    for i, m in enumerate(sorted(matches, key=lambda x: x.severity, reverse=True)):
        score -= (m.severity / 100) * 40 * _WEIGHTS.get(m.category, 1.0) * (1 / (1 + i * 0.3))
